...
function removeLeadingAndTrailingWhitespaces(value) {
  const trimmedLeft = value.trimLeft();
  const trimmedBoth = trimmedLeft.trimRight();
  return trimmedBoth;
}
...
function getRectangleString(width, height) {
  ...
  const rectangle = topBorder + horizontalLine.repeat(height - 2) + bottomBorder;
...
function encodeToRot13(string) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    ...
    let charCode13 = charCode;
    if ((charCode >= 65 && charCode <= 90) || (charCode >= 97 && charCode <= 122)) {
      const isUpperCase = (charCode >= 65 && charCode <= 90);
      charCode13 = charCode + 13;
      if ((isUpperCase && charCode13 > 90) || (!isUpperCase && charCode13 > 122)) {
        charCode13 -= 26;
      }
    }
    newString += String.fromCharCode(charCode13);
  }
  return newString;
}
/**
